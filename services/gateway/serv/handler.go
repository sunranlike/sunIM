...
import (
	...
	"github.com/sunrnalike/sun/wire/token"
	"regexp"
	"time"
)
...
func (h *Handler) Accept(conn sun.Conn, timeout time.Duration) (string, error) {
	...
	if req.Command != wire.CommandLoginSignIn {
		...
		_ = conn.WriteFrame(sun.OpBinary, pkt.Marshal(resp))
...
func (h *Handler) Receive(ag sun.Agent, payload []byte) {
	//step1:读取payload
	buf := bytes.NewBuffer(payload) //读取payload
	packet, err := pkt.Read(buf)    //读取这个payload
	if err != nil {
		log.Error(err)
		return
	}
	//step2: 类型断言是basic还是logic
	//如果是基础协议(维持心跳),既pingpong消息,直接返回pong即可
	//如果非逻辑协议(聊天消息),进行前向传输给容器,让容器
