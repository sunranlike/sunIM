...
func NewRedisStorage(cli *redis.Client) sun.SessionStorage {
	return &RedisStorage{
		cli: cli,
	}
}
...
func (r *RedisStorage) Add(session *pkt.Session) error {
	...
	err := r.cli.Set(locKey, loc.Bytes(), LocationExpired).Err() //设置过期时间
	if err != nil {
		return err
	}
	// save session
...
func (r *RedisStorage) GetLocations(accounts ...string) ([]*sun.Location, error) {
	keys := KeyLocations(accounts...)
	list, err := r.cli.MGet(keys...).Result()
	if err != nil {
		return nil, err
	}
	var result = make([]*sun.Location, 0)
	for _, l := range list {
		if l == nil {
			continue
		}
		var loc sun.Location
...
func (r *RedisStorage) GetLocation(account string, device string) (*sun.Location, error) {
